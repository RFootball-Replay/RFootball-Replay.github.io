    ...
    <>
      ...
      <div id='game-stats' className="dialog kick-player-view" style={{ zIndex: 2, opacity: 0.5, overflowY: 'hidden', position: 'absolute', left: offset.left, top: offset.top, width: divStyle.clientWidth, height: divStyle.clientHeight }}>
        <h1 id="title" className="title">
          매치 리포트{match[mtc].spaceMode && ' (space mode)'}:
          <select value={mtc} onChange={showMatch}>
            {match.map((m, index) => <option value={index} key={index} >
              {index + 1}: 레드 {match[index].scoreRed}:{match[index].scoreBlue} 블루
            </option>)}
          </select>
        ...
        <div id='leftHalf' style={{ overflowY: 'scroll', width: '50%', float: 'left', height: '100%' }}>
          <table style={{ width: '100%' }}><tbody>
            ...
            <tr>
              ...
              <td id='game-time' style={{ textAlign: 'center' }}>경기 시간: {parseTime(match[mtc].gameTicks)}</td>
              <td></td>
            </tr>
        ...
        <div id='rightHalf' style={{ position: 'absolute', left: '50%', top: '10%', overflowY: 'scroll', width: '50%', height: '90%' }}>
          ...
          <div style={{ height: '50%', overflow: 'hidden' }}>
            <p style={{ position: 'absolute', textAlign: 'center', fontSize: 18 }}>{selectedHeatmap.split(' ')[0]} - {selectedPlayer == -1 ? match[mtc].redTeam[0] : selectedPlayer}:</p>
